...
이 import는 Tinyrack utility와 theme 변수만 추가하며 root 기본 테마를 바꾸지 않습니다. 색상 utility를 쓰는 앱 root에는 `data-theme="tinyrack-dark"` 또는 `data-theme="tinyrack-light"`를 명시하세요.
...
이 preset은 daisyUI theme를 등록하지만 daisyUI의 기본 theme를 바꾸지는 않습니다. Tinyrack theme를 적용할 앱 root에 `data-theme`를 둡니다.
...
`mantine.css` import 자체는 root 기본값을 바꾸지 않고, Mantine provider가 설정한 `data-mantine-color-scheme` 범위에서만 Tinyrack CSS 변수를 적용합니다.
...
- Tinyrack Tailwind `@theme static` 토큰
- Mantine color-scheme 범위에서 공유할 Tinyrack CSS variables
...
| daisyUI 컴포넌트 + Tailwind utility | `@tinyrack/themes/tailwind/daisyui.css` | `data-theme="tinyrack-dark"` 또는 `tinyrack-light` |
