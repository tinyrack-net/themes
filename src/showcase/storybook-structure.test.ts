...
const GENERATED_STORY_MARKER = 'Generated by scripts/generate-showcase-stories.ts';
...
describe('storybook component story structure', () => {
  ...
  it('has a Storybook scenario audit script', () => {
    ...
    const auditScriptPath = join(repoRoot, 'scripts/audit-storybook-scenarios.ts');

    expect(packageJson.scripts['storybook:audit']).toBe(
      'node scripts/audit-storybook-scenarios.ts',
    );
  ...
  it('uses every public Starlight docs component in the Starlight demo and Astro fixture', () => {
    ...
    const starlightFixture = readFileSync(
      join(repoRoot, 'tests/fixtures/astro-starlight-site/src/content/docs/index.mdx'),
      'utf8',
    );
