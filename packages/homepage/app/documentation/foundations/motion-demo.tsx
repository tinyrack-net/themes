...
const copy = {
  ...
  ko: {
    ...
    reducedBody:
      '시스템 설정은 prefers-reduced-motion을 따라요. 줄이기 미리보기는 결과를 유지하면서 장식 이동을 제거해요.',
    ...
    reducedReady: '모션 줄이기 미리보기 준비',
    reducedRun: '모션 미리보기 재생 시작',
  },
  ja: {
    start: '開始',
    finish: '完了',
    durationTitle: '一つの変化、三つの反応速度',
    durationBody: '製品と同じ持続時間を使い、再生を選んだ後だけ動きます。',
    replayDuration: '持続時間を再生',
    durationLabel: 'モーション持続時間の比較',
    durationReady: '持続時間の比較準備完了',
    durationRun: '持続時間の再生を開始',
    easingTitle: '同じ時間、異なるリズム',
    easingBody:
      '曲線を比較するため 480ms を使います。製品コンポーネントは持続時間トークンを使います。',
    replayEasing: 'イージングを再生',
    easingLabel: 'モーションイージングの比較',
    easingReady: 'イージングの比較準備完了',
    easingRun: 'イージングの再生を開始',
    fast: [
      '押下に結び付くフィードバック',
      'メニュー、ツールチップ、アンカー付きレイヤー',
      'タスク文脈の変化',
    ],
    ease: ['双方向の状態変化', '画面に入るコンテンツ', '反復する進捗のみ'],
    online: 'オンライン',
    paused: '一時停止',
    toggle: 'ラック状態を切り替え',
    reducedTitle: '移動を減らし、状態変化を維持',
    reducedBody:
      'システム設定は prefers-reduced-motion に従います。低減プレビューは結果を保ちながら装飾的な移動を除きます。',
    modeLabel: 'モーションプレビューモード',
    system: 'システムモーション',
    reduced: '低減プレビュー',
    replayState: '状態変化を再生',
    complete: 'デプロイ完了',
    completeBody: '装飾的な移動を除いても結果は明確です。',
    reducedReady: 'モーション低減プレビュー準備完了',
    reducedRun: 'モーションプレビューの再生を開始',
  },
} as const;
...
export function FastFeedbackExample({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="grid gap-tinyrack-md" data-motion-fast-feedback>
      <div
        className="flex items-center justify-between gap-2 rounded-tinyrack-md border border-tinyrack-border bg-tinyrack-surface-muted p-3"
        style={{
